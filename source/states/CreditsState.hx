...
class CreditsState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		for (item in grpOptions.members)
		{
			if (!item.bold)
			{
				...
				item.x = FlxMath.lerp(item.targetY == 0 ? item.x - 70 : 200 + -40 * Math.abs(item.targetY), item.x, lerpVal);
				if (item.targetY == 0)
					item.screenCenter(X);
			}
		}
	...
	function changeSelection(change:Int = 0)
	{
		...
		var hasDesc:Bool = creditsStuff[curSelected][2].trim().length > 0;
		descText.text = creditsStuff[curSelected][2];
		descText.visible = descBox.visible = hasDesc;
	...
	function pushModCreditsToList(folder:String)
	{
		...
		if (NativeFileSystem.exists(translatedCredits))
			creditsFile = translatedCredits;
		#end

		if (NativeFileSystem.exists(creditsFile))
		{
			for (line in NativeFileSystem.getContent(creditsFile).split('\n'))
			{
